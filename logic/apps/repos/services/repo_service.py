...
def delete(name: str):
    ...
    if not repo_repository.exist(name):
        ...
        raise AppException(RepoError.SERVER_NOT_EXISTS_ERROR, msj)
...
def test_repo(name: str) -> Dict[str, str]:

    repo = get(name)
    agents = agent_service.get_by_type(repo.type)
    if not agents:
    ...
    return requests.post(url=f'{url}/api/v1/jaime/repos/test', json=json).json()


def modify(name: str, repo: Repo):
    delete(name)
    add(repo)


def update_git_repo(repo: Repo):
...
def download_git_repo(repo: Repo):
