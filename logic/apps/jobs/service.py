...
def get_all_short(size: int = 10, page: int = 1, filter: str = None, order: str = None, status: Status = None) -> List[Dict[str, str]]:
    return [
        {
            "name": w.name,
        ...
        for w in job_repository.get_all(size, page, filter, order, status)
    ]
