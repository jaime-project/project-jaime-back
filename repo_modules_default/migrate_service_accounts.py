...
for np in namespaces:
    ...
    # CLUSTERROLES
    print(f"{server_to} -> Generando work para {np} clusterroles")
    post_work(generate_yaml_params(server_from, server_to, np, 'clusterroles'))

    # CLUSTERROLEBINDINGS
    print(f"{server_to} -> Generando work para {np} clusterrolebindings")
    post_work(generate_yaml_params(
        server_from, server_to, np, 'clusterrolebindings'))
